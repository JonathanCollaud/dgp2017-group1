...
namespace mesh_processing {
    ...
    void MeshProcessing::calc_target_length(const REMESHING_TYPE &remeshing_type) {
        ...
        else if (remeshing_type == CURV)
        {
            ...
            //MeshProcessing::calc_mean_curvature() ; deja déclaré en haut
            ...
            Scalar dtl = 0.2;
 //           Mesh::Vertex_iterator v_it, v_begin, v_end; //deja déclaré en haut
 //           Mesh::Vertex_around_vertex_circulator   vv_c, vv_end;
            long             laplace(0.0);
            Scalar              n;
...
//            v_begin = mesh_.vertices_begin();
 //           v_end = mesh_.vertices_end(); Deplacé dans la boucle on sait jamais
    ...
    //cout << target_length[*v_it] << endl ;
            ...
            for (v_it = mesh_.vertices_begin(); v_it != v_end; ++v_it) { // loop over all to sum all vertex length
                sum = sum + target_length[*v_it] ;
            }
    //cout << sum << '-' << N << endl ;
            mean = sum/N ; // calculate mean length before rescaling
...
//                    cout << "Splitting the "<< j<< "th edge on iterations : " << i << endl; //DEBUGGING
	...
	void MeshProcessing::collapse_short_edges()
	{
		...
		for (finished = false, i = 0; !finished && i < 100; ++i)
		{
            ...
            j = 0; //DEBUGGING
            Scalar target_l = 0; //added for clarity
            Scalar l = 0; //added for clarity
	...
	void MeshProcessing::tangential_relaxation()
	{
        ...
        cout << "Tangential relaxation ..." << endl;
