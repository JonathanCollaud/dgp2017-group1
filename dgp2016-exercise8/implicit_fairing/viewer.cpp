...
Viewer::Viewer() : nanogui::Screen(Eigen::Vector2i(1024, 768), "DGP Viewer") {
	...
	b = new Button(window_, "Write mesh");
	b->setCallback([this]() {
		mesh_->write_mesh();
	});
	...
	initShaders();
	mesh_ = new mesh_processing::MeshProcessing("../data/icosahedron.obj");
	mesh_->load_primitives("../data/cylinder.obj", "../data/sphere.obj");
	this->refresh_mesh();
	this->refresh_trackball_center();
}
...
void Viewer::refresh_selection() {
	shaderSelection_.bind();
	Eigen::Matrix<uint32_t, Eigen::Dynamic, Eigen::Dynamic> indices = Eigen::Matrix<uint32_t, Eigen::Dynamic, Eigen::Dynamic>(2, 1);
	indices << 0, 1;
